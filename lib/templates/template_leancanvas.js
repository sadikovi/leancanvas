// Generated by CoffeeScript 1.9.0
(function() {
  var anothertag, canvas_channels, canvas_cost_structure, canvas_customer_segments, canvas_key_metrics, canvas_problem, canvas_revenue_streams, canvas_solution, canvas_unfair_advantage, canvas_unique_value_proposition, column1, column2, column3, column4, column5, column6, column7, domain1, domain2, sampletag, startups;

  this.template_leancanvas = {
    data: [
      domain1 = {
        type: "domain",
        id: "domain1",
        children: [
          column1 = {
            type: "column",
            id: "column1",
            children: [
              canvas_problem = {
                type: "directory",
                id: "canvas_problem",
                name: "Problem",
                placeholder: "Top 3 problems..."
              }
            ]
          }, column2 = {
            type: "column",
            id: "column2",
            children: [
              canvas_solution = {
                type: "directory",
                id: "canvas_solution",
                name: "Solution",
                placeholder: "Top 3 features..."
              }, canvas_key_metrics = {
                type: "directory",
                id: "canvas_key_metrics",
                name: "Key metrics",
                placeholder: "Key activities you measure..."
              }
            ]
          }, column3 = {
            type: "column",
            id: "column3",
            children: [
              canvas_unique_value_proposition = {
                type: "directory",
                id: "canvas_unique_value_proposition",
                name: "Unique value proposition",
                placeholder: "Single, clear, compelling message that states why you are different and worth buying..."
              }
            ]
          }, column4 = {
            type: "column",
            id: "column4",
            children: [
              canvas_unfair_advantage = {
                type: "directory",
                id: "canvas_unfair_advantage",
                name: "Unfair advantage",
                placeholder: "Cannot be easily copied or bought..."
              }, canvas_channels = {
                type: "directory",
                id: "canvas_channels",
                name: "Channels",
                placeholder: "Path to customers..."
              }
            ]
          }, column5 = {
            type: "column",
            id: "column5",
            children: [
              canvas_customer_segments = {
                type: "directory",
                id: "canvas_customer_segments",
                name: "Customer segments",
                placeholder: "Target customers..."
              }
            ]
          }
        ]
      }, domain2 = {
        type: "domain",
        id: "domain2",
        children: [
          column6 = {
            type: "column",
            id: "column6",
            children: [
              canvas_cost_structure = {
                type: "directory",
                id: "canvas_cost_structure",
                name: "Cost structure",
                placeholder: "Customer Acquisition Costs\r\nDistribution Costs\r\nHosting\r\nPeople, etc..."
              }
            ]
          }, column7 = {
            type: "column",
            id: "column7",
            children: [
              canvas_revenue_streams = {
                type: "directory",
                id: "canvas_revenue_streams",
                name: "Revenue streams",
                placeholder: "Revenue Model\r\nLife Time Value\r\nRevenue\r\nGross Margin..."
              }
            ]
          }
        ]
      }
    ],
    tags: [
      sampletag = {
        id: "sample123",
        name: "important",
        type: "tag",
        color: "red"
      }, anothertag = {
        id: "idea123",
        name: "idea",
        type: "tag",
        color: "yellow"
      }, startups = {
        id: "cookie123",
        name: "cookie",
        type: "tag",
        color: "purple"
      }
    ]
  };

}).call(this);
